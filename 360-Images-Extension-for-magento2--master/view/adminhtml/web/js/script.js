...
define(['jquery'], function($){
    return {
        ...
        initSwitcher: function(){
            $('.imagetoolbox-switcher-status').on('change', function(event) {
                ...
                if (disabled) {
                    var name = elements.prop('name').replace(/(imagetoolbox\[[^\]]+\])\[[^\]]+\](\[[^\]]+\])/, '$1[default]$2');
                    var value;
        ...
        initDefaults: function(){
            ...
            $('#page_tabs_default_content .control').find('input, select').bind('change', function(){
                ...
                for (var profileIndex in profiles) {
                    ...
                    var curName = name.replace(/(imagetoolbox\[[^\]]+\])\[default\](\[[^\]]+\])/, '$1['+profiles[profileIndex]+']$2');
